...
function convertRemoteToLocalModel(
  ...
  if (providers.length > 0) {
    baseModel.available_providers = providers as ModelSpec["available_providers"];
  }
  ...
  try {
    ...
    for (const item of modelsToCompare) {
      ...
      if (remoteProviders.length > 0) {
        const currentProviders = (modelInUpdatedList as any).available_providers;
        const same =
          Array.isArray(currentProviders) &&
          currentProviders.length === remoteProviders.length &&
          currentProviders.every((p: string, i: number) => p === remoteProviders[i]);
        if (!same) {
