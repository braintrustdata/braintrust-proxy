...
it("forces streaming-only OpenAI-compatible models onto the stream path", async () => {
  const encoder = new TextEncoder();
  const requests: Array<{ body: JsonBodyType | null }> = [];
    ...
    return new Response(
      new ReadableStream({
        start(controller) {
          controller.enqueue(
            encoder.encode(
              'data: {"id":"chatcmpl-stream-only","object":"chat.completion.chunk","created":123,"model":"stream-only-model","choices":[{"index":0,"delta":{"role":"assistant","content":"Hello"},"finish_reason":null}]}\n\n',
            ),
          );
          controller.enqueue(
            encoder.encode(
              'data: {"id":"chatcmpl-stream-only","object":"chat.completion.chunk","created":123,"model":"stream-only-model","choices":[{"index":0,"delta":{"content":" world"},"finish_reason":"stop"}],"usage":{"prompt_tokens":3,"completion_tokens":2,"total_tokens":5}}\n\n',
            ),
          );
  ...
  const { events, headers } = await callProxyV1<
    OpenAIChatCompletionCreateParams,
    OpenAIChatCompletionChunk
  >({
    body: {
  ...
  expect(streamedEvents[0].data.choices[0]?.delta?.content).toBe("Hello");
  expect(streamedEvents[1].data.choices[0]?.delta?.content).toBe(" world");
  expect(streamedEvents[1].data.choices[0]?.finish_reason).toBe("stop");
});
