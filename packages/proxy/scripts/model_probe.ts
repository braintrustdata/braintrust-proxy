...
export const PROVIDER_APIS: Record<string, ProviderApi> = {
  ...
  // probe). We list ALL models (no endpoint filter) so a model still served on
  // any endpoint is not falsely deprecated. Cohere ids are bare names, matching
  // catalog keys directly (no canonicalization needed).
  cohere: {
    listIsAuthoritative: true,
    probeModel: null,
    listModels: async (secret) => {
